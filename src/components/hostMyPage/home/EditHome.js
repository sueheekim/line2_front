...
function EditHome(props) {
    ...
    useEffect(() => {
        ...
        axios.get(homeUrl + '/' + id).then(res => {
            ...
            document.getElementById(
                `homeCategory${res.data.homeCategoryId}`,
            ).selected = true;
            document.getElementById('homePolicyCustom').value =
                res.data.homePolicyCustom;
            document.getElementById('homeInformation').value =
            ...
            res.data.rooms &&
                res.data.rooms.map((room, index) => {
                    document.getElementById(
                        `home_edit_room_name${index}`,
                    ).value = room.roomName;
                    document.getElementById(
                        `home_edit_room_gender${index}`,
                    ).value = room.gender;
                    document.getElementById(
                        `home_edit_room_single_bed${index}`,
                    ).value = room.singleBed;
                    document.getElementById(
                        `home_edit_room_double_bed${index}`,
                    ).value = room.doubleBed;
                    document.getElementById(
                        `home_edit_room_bedding${index}`,
                    ).value = room.bedding;
                });
        });
    }, []);
    ...
    useEffect(() => {
        if (rooms.length !== 0) {
            ...
            document.getElementById(`home_edit_room_name${roomIndex}`).value =
                rooms[roomIndex].roomName;
            document.getElementById(`home_edit_room_gender${roomIndex}`).value =
                rooms[roomIndex].gender;
            document.getElementById(
                `home_edit_room_single_bed${roomIndex}`,
            ).value = rooms[roomIndex].singleBed;
            document.getElementById(
                `home_edit_room_double_bed${roomIndex}`,
            ).value = rooms[roomIndex].doubleBed;
            document.getElementById(
                `home_edit_room_bedding${roomIndex}`,
            ).value = rooms[roomIndex].bedding;
        }
    }, [rooms]);
                ...
                axios.get(isEnableDeleteRoomUrl + rooms[index].id).then(res => {
                    if (res.data.code === 1) {
                        setRooms(
                            rooms
                                .slice(0, index)
                                .concat(rooms.slice(index + 1, rooms.length)),
                        );
    ...
    const addImageButton = () => {
        setImages([
            ...images,
            document.getElementById('edit_home_add_image').files[0].name,
        ]);
    };
    ...
    const checkRoom = () => {
        ...
        rooms.map((room, index) => {
            roomsTmp.push({
                id: room.id,
                roomName: document.getElementById(`home_edit_room_name${index}`)
                    .value,
                gender: document.getElementById(`home_edit_room_gender${index}`)
                    .value,
                singleBed: document.getElementById(
                    `home_edit_room_single_bed${index}`,
                ).value,
                doubleBed: document.getElementById(
                    `home_edit_room_double_bed${index}`,
                ).value,
                bedding: document.getElementById(
                    `home_edit_room_bedding${index}`,
                ).value,
                maxHeadCount:
                    parseInt(
        ...
        <>
            ...
            <div className={'container'}>
                ...
                <div className={'contents_container'}>
                                    ...
                                    <div key={index}>
                                        <div>
                                            <div className={'home_edit_room'}>
                                                ...
                                                <button
                                                    className={
                                                        'home_edit_delete_button'
                                                    }
                                                    onClick={() =>
                                                        deleteRoomButton(index)
                                                    }
                                                >
                                                    객실 삭제
                                                </button>
                                        ...
                                        <div>
                                            <p>숙박 가능한 성별</p>
                                            <select
                                                id={`home_edit_room_gender${index}`}
                                                className={'home_edit_select'}
                                            >
                                                <option value={true}>
                                                    남자
                    ...
                    <div className={'row'}>
                        <div className={'edit_home_small_title'}>
                            체크인 정책
                        </div>
                        <div className={'edit_home_small_content'}>
                            <div>
                            ...
                            <div className={'row'}>
                                {images &&
                                    images.map((image, index) => (
                                        <div
                                            className={'edit_home_images_box'}
                                            key={index}
                                        >
                                            <img
                                                className={'edit_home_image'}
                                                src={`/img/${image}`}
                                                alt={image}
                                            />
                                            <button
                                                className={
                                                    'edit_home_image_delete_button'
                                                }
                                                onClick={() =>
                ...
                <div className={'row'}>
                    <button
                        className={'home_edit_delete_home_button'}
                        onClick={() => deleteHomeButton()}
                    >
                        숙소 삭제
                    </button>
                    <button
                        className={'home_edit_edit_home_button'}
                        onClick={() => editHomeButton()}
                    >
                        정보 수정 완료
                    </button>
                </div>
